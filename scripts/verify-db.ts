...
async function main() {
    ...
    const gistIndexes = await prisma.$queryRaw<{ indexname: string }[]>`SELECT indexname FROM pg_indexes WHERE indexdef ILIKE '%USING gist%'`;
    const expectedIndexes = ["buildings_geometry_idx", "departments_geometry_idx", "pois_geometry_idx"];
    ...
    const triggers = await prisma.$queryRaw<{ tgname: string }[]>`SELECT tgname FROM pg_trigger WHERE tgname = 'trg_poi_inherit_building'`;
    ...
    console.log("");
    if (hasErrors) {
        console.log("La verificación encontró problemas. Revisa los puntos marcados con ✗ arriba.");
        process.exitCode = 1;
    } 
    else {
        console.log("Todo el ambiente de base de datos está correctamente configurado.");
    }
}
// Ejecutar la función principal y manejar errores
main().catch((error) => {console.error("Error inesperado durante la verificación:", error);process.exitCode = 1;}).finally(async () => {await prisma.$disconnect();});
